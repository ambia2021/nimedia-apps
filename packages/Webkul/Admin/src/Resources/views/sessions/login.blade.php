...
@section('content')
    <div class="flex w-1/2 bg-gradient-to-tr from-blue-800 to-blue-400 i justify-around items-center">
        <div>
    ...
    <div class="flex w-1/2 justify-center items-center bg-white">
        <form method="POST" action="{{ route('admin.session.store') }}" @submit.prevent="$root.onSubmit">
            ...
            <h1 class="text-gray-800 font-bold text-2xl mb-1">Hallo Selamat datang</h1>
            ...
            <div :class="[errors.has('email') ? 'has-error' : '']"
                ...
                <input class="pl-2 outline-none border-none" type="email" name="email" class="control" id="email"
                    ...
                    data-vv-as="&quot;{{ __('admin::app.sessions.login.email') }}&quot;" placeholder="Email Address" />
            ...
            <div class="flex items-center border-2 py-2 px-3 rounded-xl focus-within:ring-2 focus-within:ring-indigo-400">
                ...
                <input class="pl-2 outline-none border-none" type="password" name="password" class="control" id="password"
                    v-validate.disable="'required|min:6'"
                    data-vv-as="&quot;{{ __('admin::app.sessions.login.password') }}&quot;" placeholder="Password" />
            ...
            <button type="submit"
                class="block w-full bg-blue-600 hover:bg-indigo-900 mt-4 py-2 rounded-2xl text-white font-semibold mb-2">Login</button>
            {!! view_render_event('admin.sessions.login.form_buttons.after') !!}

            <div class="flex justify-between">
            </form>
            <span> <a class="text-sm ml-2 hover:text-blue-800  cursor-pointer text-slate-900"
                href="{{ route('admin.forgot_password.create') }}">lupa password?</a></span>
                <span> 
                    <a class="text-sm hover:text-blue-800 cursor-pointer item-right flex-none"
                    href="{{ route('admin.register.index') }}">buat akun?</a>
                </span>
            </div>
            </div>
